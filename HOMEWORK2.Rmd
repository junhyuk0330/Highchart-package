...
survey18<-read.csv("/home/js/R/R-skku-biohrs/survey_results_public.csv")
```

## INTRO

<center>
![**How to visualize**](https://blogs.sap.com/wp-content/uploads/2016/12/chart.jpg){width=80%}
</center>
...
- 그러나, 데이터를 이해하여 그것을 가공해 활용할 수 없다면</br> </br>데이터는 무의미하다. 
</br>
</br>
...
<center>
![**highchart**](https://wp-assets.highcharts.com/svg/logo_mark.svg){width=60%}
</center>
...
- **예시용**: survey_results_public.csv
</br>
</br>
- **실습용**: example_g1e.csv + 내재된 데이터(mtcars, iris 등)
</br>
</br>
dt<-fread("https://raw.githubusercontent.com/jinseob2kim/lecture-snuhlab/master/data/example_g1e.csv")
...
- type of chart 에 **column, bar, line** 등을 기입
</br>
</br>
</br>
- 가장 기본적인 그래프 + 활용하기 가장 용이함
...
hchart(penguins, 'scatter', hcaes(x=body_mass_g, y=flipper_length_mm, group=species)) %>% 
  hc_colors(c("white","yellow","pink")) %>% 
  hc_title(text="Mass and Length By Species") %>% 
...
- **example_g1e.csv** 파일을 불러옴

</br>
- **select** 함수를 이용해서 **EXMD_BZ_YYY, HGHT, WGHT, BMI**

변수만 추출

</br>
- 데이터 안에 각 연도별 데이터가 몇개씩 존재하는지

확인하는 그래프 그리기. (**count** 함수 이용)

</br>
- **highchart** 그리기. 차트 형태는 **column**
